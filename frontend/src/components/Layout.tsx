...
import {
  ...
  Building2, Settings, Users, HardHat, Calendar, Building, Globe
} from 'lucide-react';
...
const Layout = () => {
  ...
  const platformNavItems: NavItem[] = [
    { label: 'Platform Dashboard', path: '/platform', icon: Globe },
    { label: 'Tenants', path: '/platform/tenants', icon: Building2 },
  ];

  const filteredNav = isPlatformAdmin
    ? []
    : navItems.filter(item => !item.roles || item.roles.includes(user?.role || ''));
  const filteredPlatformNav = isPlatformAdmin
    ? platformNavItems.filter(item => !item.roles || isPlatformAdmin)
    : [];
    ...
    <div className="flex h-screen bg-background">
      ...
      <aside className={`${sidebarOpen ? 'w-64' : 'w-16'} bg-sidebar text-sidebar-foreground transition-all duration-200 flex flex-col border-r`}>
        ...
        <div className="flex items-center justify-between px-4 py-4">
          {sidebarOpen && (
            <Link to={isPlatformAdmin ? '/platform' : '/dashboard'} className="flex items-center gap-2">
        ...
        <nav className="flex-1 py-2 overflow-y-auto">
          {(isPlatformAdmin ? filteredPlatformNav : filteredNav).map(item => {
